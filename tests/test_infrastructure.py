...
from app.infrastructure.parsers.rss_parser import RSSParser, RSSNewsItem
...
@pytest.mark.asyncio
async def test_openai_generator_success(mocker):
...
@pytest.mark.asyncio
async def test_openai_generator_empty_response(mocker):
    mock_chat_completion = MagicMock()
...
@pytest.mark.asyncio
async def test_openai_generator_openai_error(mocker):
    mock_openai_client = AsyncMock()
...
# 2. RSS Parser Tests
@pytest.mark.asyncio
async def test_rss_parser_fetch_feed_success(mocker):
...
@pytest.mark.asyncio
async def test_rss_parser_fetch_feed_exception(mocker):
    mock_http_client = AsyncMock()
...
@pytest.mark.asyncio
async def test_telegram_channel_parser_fetch_messages(mocker):
...
@pytest.mark.asyncio
async def test_telegram_channel_parser_fetch_messages_error(mocker):
    mock_parser_client = MagicMock()
...
def test_telegram_channel_parser_filter_by_keywords():
...
def test_telegram_parser_client_init_validation():
    with pytest.raises(ValueError):
        TelegramParserClient(api_id=0, api_hash="hash", session_string="string")

@pytest.mark.asyncio
async def test_telegram_parser_client_connect_and_authorize(mocker):
    mock_tc = AsyncMock()
...
@pytest.mark.asyncio
async def test_telegram_parser_client_unauthorized(mocker):
    mock_tc = AsyncMock()
    mock_tc.connect = AsyncMock()
...
@pytest.mark.asyncio
async def test_telegram_publisher_success(mocker):
    mock_bot = AsyncMock()
...
@pytest.mark.asyncio
async def test_telegram_publisher_exception(mocker):
    mock_bot = AsyncMock()
